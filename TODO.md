...
### 🚨 現在の課題：データ不足問題（2026-02-08確認）

**現状**:
- 全銘柄数: 626社（1日分のEDINET提出のみ）
- 売上データあり: 20社（3.2%）
- 純利益/純資産/総資産データあり: 3社のみ（0.5%）
- 株価データあり: 460社（Stooq取得成功）

**原因**:
1. **銘柄数が少ない**: EDINETから特定日1日分しかデータ取得していない
2. **財務データが"-"**: XBRLパースで財務データを正しく抽出できていない
   - 正規表現パターンがEDINETのXBRL形式と合っていない可能性
   - データが存在するファイル（決算短信vs有価証券報告書）の違い

**対策TODO**:
- [ ] **過去1年分のEDINETデータを一括取得**するバッチ処理
- [ ] **XBRLパースの正規表現を改善**（EDINETの実際のタグ形式を調査）
- [ ] **有価証券報告書（Annual Reports）を優先的に取得**
- [ ] **テスト用にXBRLファイルをダウンロードして解析ログを出力**
- [ ] **日本株全銘柄リスト**を外部から取得（証券コード一覧）
